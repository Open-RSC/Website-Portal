...
update-laravel:
	docker exec -i php bash -c "cd /var/www/html/portal && composer install && composer update && php artisan key:generate && php artisan optimize && npm install && npm update && npm audit fix"
