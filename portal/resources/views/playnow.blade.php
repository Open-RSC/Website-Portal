...
@section('content')
    <div class="play-now-container">
        <a href="/playnow/worldlist" class="play-now-left-button"
            aria-label="Play OpenRSC servers via the web client"></a>
        <a href="{{ $gameClientUrl }}" class="play-now-right-button"
            aria-label="Play OpenRSC servers via the {{ $gameClientName }}"></a>
        <img src="{{ $graphicImageUrl }}" class="play-now-image"
            alt="Knight and Magician present two options for playing OpenRSC servers" />
    </div>
