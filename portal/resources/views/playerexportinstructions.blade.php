...
@section('content')
    <div class="col container">
        <h2 class="h2 text-center text-gray-400 pt-5 pb-4 text-capitalize display-3">How to use OpenRSC player exports!</h2>
        ...
        <h3>Introduction</h3>
        <p>OpenRSC player exports include a GPG signed zip archive of the <code>metadata.txt</code>, <code>playerdata.db</code>, and <code>playerdata.sql</code> files, in a file which is always called <code>data.zip.gpg</code></p>
        <p>By using GPG, you can verify that the account data has not been altered since it was exported from OpenRSC.</p>

        <h3>How to extract</h3>
        <ol>
            <li>First, you will need the public key associated with the private key that OpenRSC used to sign the data. A link to this file is included in <code>metadata.txt</code>.</li>
            <li>Ensure that you trust that the GPG Link looks like it belongs to OpenRSC. We have used https://rsc.vet and https://openrsc.com at the time of this writing.</li>
            ...
            <li>You will want to inform GPG how much you trust that data signed with the private key associated with that public key comes from OpenRSC.</li>
            ...
            <li>If the player export is legitimately from OpenRSC, you will receive the message "<code>gpg: Good signature from "OpenRSC (Email address on this GPG key will probably not work for contacting us. Our admins' individual email addresses should be on git commits by Ken, Kenix, Logg, Luis, Marwolf or Ryan. If Discord still exists when you're reading this message, that's the best way to reach us, as of 2023-02-16!) <openrsc.emailer@gmail.com>" [ultimate]</code>" as part of the extraction output.</li>
        </ol>
        ...
        <p>Thanks on behalf of the entire OpenRSC Team,</p>
        <p>&mdash; Logg</p>
    </div>
@endsection
@section('scripts')
